...
type Configuration struct {
	...
	EchoSiteId    string `yaml:"echosite_id,omitempty"`
	EchoSiteToken string `yaml:"echosite_token,omitempty"`
}
...
func LoadConfiguration(opts *Options) (config *Configuration, err error) {
	...
	if config.ServerAddr, err = normalizeAddress(config.ServerAddr, "server_addr"); err != nil {
		...
		config.ServerAddr = "echositecn." + config.ServerAddr
	}
	...
	if opts.echoSiteId != "" {
		config.EchoSiteId = opts.echoSiteId
	}
	if opts.echoSiteToken != "" {
		config.EchoSiteToken = opts.echoSiteToken
	}
