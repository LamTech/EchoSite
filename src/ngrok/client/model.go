...
func newClientModel(config *Configuration, ctl mvc.Controller) *ClientModel {
	...
	m := &ClientModel{
		...
		echoSiteId:    config.EchoSiteId,
		echoSiteToken: config.EchoSiteToken,
	}
...
func (c *ClientModel) control() {
	...
	auth := &msg.Auth{
		...
		User:          c.authToken,
		EchoSiteId:    c.echoSiteId,
		EchoSiteToken: c.echoSiteToken,
	}
	...
	if authResp.Error != "" {
		emsg := fmt.Sprintf("用户验证失败: %s", authResp.Error)
		c.ctl.Shutdown(emsg)
		return
	}
	...
	for {
		...
		case *msg.NewTunnel:
			if m.Error != "" {
				emsg := fmt.Sprintf("隧道分配失败: %s", m.Error)
				c.Error(emsg)
				c.ctl.Shutdown(emsg)
				continue
			}
			...
			c.Info("隧道注册成功 %v", tunnel.PublicUrl)
			c.update()

		default:
			ctlConn.Warn("未知消息 %v ", m)
		}
	}
}
...
func (c *ClientModel) proxy() {
	...
	if err != nil {
		remoteConn.Error("隧道代理注册消息发送失败: %v", err)
		return
	}
