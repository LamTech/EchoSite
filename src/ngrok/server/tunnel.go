...
func NewTunnel(m *msg.ReqTunnel, ctl *Control) (t *Tunnel, err error) {
	...
	if esr, err := echosite.Request(echosite.EsReq{
		...
		Data: map[string]interface{}{
			"EchoSiteId": ctl.account.Id,
			"Protocol":   t.req.Protocol,
			"Subdomain":  t.req.Subdomain,
			"Hostname":   t.req.Hostname,
			"RemotePort": t.req.RemotePort,
		},
	}); err != nil {
		log.Error(err.Error())
		return nil, fmt.Errorf("EchoSite Server Err!")
	} else {
		if esr.Code != 0 {
			err := fmt.Errorf("%d,%s", esr.Code, esr.Message)
			t.ctl.conn.Error("%v", err)
			return nil, err
		}
	}
