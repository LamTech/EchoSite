...
func NewControl(ctlConn conn.Conn, authMsg *msg.Auth) {
	...
	c := &Control{
		...
		account: &echosite.EsUser{
			Id:    authMsg.EchoSiteId,
			Token: authMsg.EchoSiteToken,
		},
	}
	...
	if esr, err := echosite.Request(echosite.EsReq{
		Action: "AUTH_TOKEN",
		Data: map[string]interface{}{
			"EchoSiteId":    c.account.Id,
			"EchoSiteToken": c.account.Token,
		},
	}); err != nil {
		c.conn.Error(err.Error())
		failAuth(fmt.Errorf("EchoSite Server Err!"))
	} else {
		if esr.Code != 0 {
			failAuth(fmt.Errorf("%d,%s", esr.Code, esr.Message))
			return
		}
	}
	...
	if authMsg.Version != version.Proto {
		failAuth(fmt.Errorf("Incompatible versions. Server %s, client %s. Download a new version at http://www.echosite.cn", version.MajorMinor(), authMsg.Version))
		return
	}
